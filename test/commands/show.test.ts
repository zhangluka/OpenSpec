...
describe("top-level show command", () => {
  ...
  const changesDir = path.join(testDir, "phspec", "changes");
  const specsDir = path.join(testDir, "phspec", "specs");
  const openspecBin = path.join(projectRoot, "bin", "phspec.js");
  ...
  it("prints hint and non-zero exit when no args and non-interactive", () => {
    ...
    try {
      ...
      expect(stderr).toContain("未指定查看对象");
      expect(stderr).toContain("phspec show <item>");
      expect(stderr).toContain("phspec change show");
      expect(stderr).toContain("phspec spec show");
    } finally {
  ...
  it("auto-detects change id and supports --json", () => {
    ...
    try {
      process.chdir(testDir);
      const output = execSync(`node ${openspecBin} show demo --json`, {
        encoding: "utf-8",
      });
  ...
  it("auto-detects spec id and supports spec-only flags", () => {
    ...
    try {
      ...
      const output = execSync(
        `node ${openspecBin} show auth --json --requirements`,
        { encoding: "utf-8" },
      );
      const json = JSON.parse(output);
  ...
  it("prints nearest matches when not found", () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${openspecBin} show unknown-item`, {
          encoding: "utf-8",
        });
      } catch (e) {
        err = e;
