  ...
  describe("spec show", () => {
    ...
    it("should output spec as JSON with --json flag", () => {
      ...
      try {
        ...
        expect(json.metadata.format).toBe("phspec");
      } finally {
        process.chdir(originalCwd);
      }
    });
    ...
    it("should filter to show only requirements with --requirements flag (JSON only)", () => {
      ...
      try {
        ...
        const output = execSync(
          `node ${openspecBin} spec show auth --json --requirements`,
          {
            encoding: "utf-8",
          },
        );
    ...
    it("should exclude scenarios with --no-scenarios flag (JSON only)", () => {
      ...
      try {
        process.chdir(testDir);
        const output = execSync(
          `node ${openspecBin} spec show auth --json --no-scenarios`,
          {
            encoding: "utf-8",
          },
        );
    ...
    it("should show specific requirement with -r flag (JSON only)", () => {
      ...
      try {
        process.chdir(testDir);
        const output = execSync(
          `node ${openspecBin} spec show auth --json -r 1`,
          {
            encoding: "utf-8",
          },
        );
  ...
  describe("spec validate", () => {
    ...
    it("should output validation report as JSON with --json flag", () => {
      ...
      try {
        process.chdir(testDir);
        const output = execSync(
          `node ${openspecBin} spec validate auth --json`,
          {
            encoding: "utf-8",
          },
        );
    ...
    it("should validate with strict mode", () => {
      ...
      try {
        process.chdir(testDir);
        const output = execSync(
          `node ${openspecBin} spec validate auth --strict --json`,
          {
            encoding: "utf-8",
          },
        );
...
This section has no actual requirements`;
      ...
      await fs.writeFile(
        path.join(specsDir, "invalid", "spec.md"),
        invalidSpec,
      );
  ...
  describe("error handling", () => {
    ...
    it("should honor --no-color (no ANSI escapes)", () => {
      ...
      try {
        process.chdir(testDir);
        const output = execSync(
          `node ${openspecBin} --no-color spec list --long`,
          { encoding: "utf-8" },
        );
