...
describe('tool-detection', () => {
  ...
  describe('SKILL_NAMES', () => {
    it('should contain all 13 skill names', () => {
      expect(SKILL_NAMES).toHaveLength(13);
      expect(SKILL_NAMES).toContain('phspec-explore');
      expect(SKILL_NAMES).toContain('phspec-new-change');
      expect(SKILL_NAMES).toContain('phspec-continue-change');
      expect(SKILL_NAMES).toContain('phspec-apply-change');
      expect(SKILL_NAMES).toContain('phspec-ff-change');
      expect(SKILL_NAMES).toContain('phspec-sync-specs');
      expect(SKILL_NAMES).toContain('phspec-archive-change');
      expect(SKILL_NAMES).toContain('phspec-bulk-archive-change');
      expect(SKILL_NAMES).toContain('phspec-verify-change');
      expect(SKILL_NAMES).toContain('phspec-review-spec');
      expect(SKILL_NAMES).toContain('phspec-review-code');
      expect(SKILL_NAMES).toContain('phspec-review-design');
      expect(SKILL_NAMES).toContain('phspec-onboard');
    });
  });
  ...
  describe('getToolSkillStatus', () => {
    ...
    it('should detect when one skill exists', async () => {
      const skillDir = path.join(testDir, '.claude', 'skills', 'phspec-explore');
    ...
    it('should detect when all skills exist', async () => {
      ...
      expect(status.skillCount).toBe(13);
    });
  });
...
`);
  ...
  describe('getConfiguredTools', () => {
    ...
    it('should return configured tools', async () => {
      // Setup Claude
      const claudeSkillDir = path.join(testDir, '.claude', 'skills', 'phspec-explore');
      await fs.mkdir(claudeSkillDir, { recursive: true });
      await fs.writeFile(path.join(claudeSkillDir, 'SKILL.md'), 'content');

      // Setup Cursor
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'phspec-explore');
