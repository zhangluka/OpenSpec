...
describe("CompletionProvider", () => {
  ...
  describe("getChangeIds", () => {
    ...
    it("should return active change IDs", async () => {
      ...
      await fs.writeFile(
        path.join(changesDir, "change-1", "proposal.md"),
        "# Change 1",
      );

      await fs.mkdir(path.join(changesDir, "change-2"), { recursive: true });
      await fs.writeFile(
        path.join(changesDir, "change-2", "proposal.md"),
        "# Change 2",
      );
    ...
    it("should exclude archive directory", async () => {
      ...
      await fs.writeFile(
        path.join(changesDir, "active-change", "proposal.md"),
        "# Active",
      );

      // Create archived change
      await fs.mkdir(path.join(changesDir, "archive", "old-change"), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(changesDir, "archive", "old-change", "proposal.md"),
        "# Old",
      );
