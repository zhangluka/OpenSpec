...
describe("PowerShellInstaller", () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(
      os.tmpdir(),
      `phspec-powershell-test-${randomUUID()}`,
    );
  ...
  describe("getInstallationPath", () => {
    it("should return path relative to profile directory", () => {
      ...
      expect(result).toBe(
        path.join(testHomeDir, ".config", "powershell", "PhSpecCompletion.ps1"),
      );
    });
    ...
    it("should work with custom PROFILE environment variable", () => {
      ...
      const result = installer.getInstallationPath();
      expect(result).toBe(
        path.join(testHomeDir, "custom", "PhSpecCompletion.ps1"),
      );
    });
  ...
  describe("backupExistingFile", () => {
    ...
    it("should copy file content to backup", async () => {
      ...
      const originalContent =
        "# Original PowerShell completion script\n$completer = {}";
  ...
  describe("configureProfile", () => {
    const mockScriptPath = "/path/to/PhSpecCompletion.ps1";

    // Note: PHSPEC_NO_AUTO_CONFIG check is now handled in the install() method,
  ...
  describe("removeProfileConfig", () => {
    ...
    it("should remove content between markers", async () => {
      ...
      expect(content).not.toContain("# PhSpec completions");
  ...
  describe("install", () => {
    const mockCompletionScript = `# PowerShell completion script for PhSpec
$phspecCompleter = {
...
`;
    ...
    it("should install completion script for the first time", async () => {
      ...
      expect(result.installedPath).toContain("PhSpecCompletion.ps1");
    ...
    // Note: PHSPEC_NO_AUTO_CONFIG support was removed from PowerShell installer
    ...
    it.skipIf(process.platform === "win32")(
      ...
      async () => {
        ...
        expect(result.instructions!.some((i) => i.includes("Test-Path"))).toBe(
          true,
        );
      },
    );
    ...
    it("should handle installation with paths containing spaces", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain("phspec powershell test");
