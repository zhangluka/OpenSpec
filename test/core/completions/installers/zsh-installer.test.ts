...
describe("ZshInstaller", () => {
  ...
  beforeEach(async () => {
    // Unset ZSH so isOhMyZshInstalled() only returns true when test creates .oh-my-zsh in testHomeDir
    savedZsh = process.env.ZSH;
    delete process.env.ZSH;
    // Create a temporary home directory for testing
    testHomeDir = path.join(os.tmpdir(), `phspec-zsh-test-${randomUUID()}`);
    await fs.mkdir(testHomeDir, { recursive: true });
    installer = new ZshInstaller(testHomeDir);
  });

  afterEach(async () => {
    if (savedZsh !== undefined) process.env.ZSH = savedZsh;
    else delete process.env.ZSH;
  ...
  describe("getInstallationPath", () => {
    ...
    it("should return standard Zsh path when Oh My Zsh is not installed", async () => {
      ...
      expect(result.path).toBe(
        path.join(testHomeDir, ".zsh", "completions", "_phspec"),
      );
    });
  });
  ...
  describe("install", () => {
    const testScript = '#compdef phspec\n_phspec() {\n  echo "test"\n}\n';
    ...
    it("should install to Oh My Zsh path when Oh My Zsh is present", async () => {
      ...
      expect(result.installedPath).toBe(
        path.join(ohMyZshPath, "custom", "completions", "_phspec"),
      );
    ...
    it("should detect already-installed completion with identical content", async () => {
      ...
      expect(secondResult.instructions!.join(" ")).toContain(
        "already installed",
      );
    });
    ...
    it("should handle paths with spaces in .zshrc config", async () => {
      ...
      try {
        ...
        try {
          ...
          expect(zshrcContent).toContain(
            `fpath=("${path.dirname(result.installedPath!)}" $fpath)`,
          );
        } catch {
  ...
  describe("uninstall", () => {
    ...
    it("should remove from correct location for Oh My Zsh", async () => {
      ...
      expect(result.message).toContain(
        path.join(".oh-my-zsh", "custom", "completions", "_phspec"),
      );
    });
  });
  ...
  describe("getInstallationInfo", () => {
    ...
    it("should return installation info when installed", async () => {
      ...
      expect(info.path).toContain("_phspec");
  ...
  describe("removeZshrcConfig", () => {
    ...
    it("should remove markers and config when present", async () => {
      ...
      expect(newContent).not.toContain("PhSpec shell completions");
  ...
  describe("install with .zshrc auto-configuration", () => {
    ...
    it("should update success message when .zshrc is configured", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toMatch(/\.zshrc configured|Oh My Zsh/);
    });
  });
  ...
  describe("uninstall with .zshrc cleanup", () => {
    ...
    it("should remove .zshrc config when uninstalling", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain(
        "Removed PhSpec configuration from ~/.zshrc",
      );
