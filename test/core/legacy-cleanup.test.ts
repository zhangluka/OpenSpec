...
${OPENSPEC_MARKERS.end}`,
    ...
    it("should detect multiple config files", async () => {
      ...
      await fs.writeFile(
        path.join(testDir, "CLINE.md"),
        `${OPENSPEC_MARKERS.start}\nContent\n${OPENSPEC_MARKERS.end}`,
      );
      await fs.writeFile(
        path.join(testDir, "QODER.md"),
        `${OPENSPEC_MARKERS.start}\nContent\n${OPENSPEC_MARKERS.end}`,
      );
  ...
  describe("detectLegacySlashCommands", () => {
    ...
    it("should detect multiple tool directories and files", async () => {
      ...
      await fs.mkdir(path.join(testDir, ".qoder", "commands", "openspec"), {
        recursive: true,
      });
    ...
    it("should detect TOML-based slash commands for Qwen", async () => {
      ...
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(
        path.join(dirPath, "openspec-proposal.toml"),
        "content",
      );
    ...
    it("should detect Continue prompt files", async () => {
      ...
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(
        path.join(dirPath, "openspec-apply.prompt"),
        "content",
      );
...
${OPENSPEC_MARKERS.end}`,
    ...
    it("should delete legacy slash command directories", async () => {
      ...
      await expect(
        fs.access(path.join(testDir, ".claude", "commands")),
    ...
    it("should delete openspec/AGENTS.md", async () => {
      ...
      await expect(
        fs.access(path.join(testDir, "openspec")),
      ).resolves.not.toThrow();
    });
...
${OPENSPEC_MARKERS.end}`,
  ...
  describe("LEGACY_CONFIG_FILES", () => {
    it("should include expected config file names", () => {
      ...
      expect(LEGACY_CONFIG_FILES).toContain("DEVAGENT.md");
  ...
  describe("LEGACY_SLASH_COMMAND_PATHS", () => {
    ...
    it("should cover all tools from the CommandAdapterRegistry", () => {
      const expectedTools = CommandAdapterRegistry.getAll().map(
        (adapter) => adapter.toolId,
      );
      ...
      // Verify counts match
      expect(expectedTools.length).toBe(
        Object.keys(LEGACY_SLASH_COMMAND_PATHS).length,
      );
    });
  });
