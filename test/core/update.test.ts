...
describe("UpdateCommand", () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `phspec-test-${randomUUID()}`);
    await fs.mkdir(testDir, { recursive: true });

    // Create phspec directory
    const phspecDir = path.join(testDir, "phspec");
    await fs.mkdir(phspecDir, { recursive: true });
  ...
  describe("basic validation", () => {
    it("should throw error if phspec directory does not exist", async () => {
      // Remove phspec directory
      await fs.rm(path.join(testDir, "phspec"), {
        recursive: true,
        force: true,
      });

      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "未找到 PhSpec 目录，请先执行 'phspec init'。",
      );
    });
...
`;
      ...
      expect(updatedSkill).toContain("name: phspec-explore");
    ...
    it("should update all 9 skill files when tool is configured", async () => {
      ...
      await fs.mkdir(path.join(skillsDir, "phspec-explore"), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, "phspec-explore", "SKILL.md"),
        "old content",
      );
  ...
  describe("multi-tool support", () => {
    it("should update multiple configured tools", async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, "phspec-explore"), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, "phspec-explore", "SKILL.md"),
        "old",
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, ".cursor", "skills");
      await fs.mkdir(path.join(cursorSkillsDir, "phspec-explore"), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, "phspec-explore", "SKILL.md"),
        "old",
      );
      ...
      expect(claudeSkill).toContain("name: phspec-explore");
      ...
      expect(cursorSkill).toContain("name: phspec-explore");
    ...
    it("should update Qwen tool with correct command format", async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, "phspec-explore"), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, "phspec-explore", "SKILL.md"),
        "old",
      );
    ...
    it("should update Windsurf tool with correct command format", async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, "phspec-explore"), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, "phspec-explore", "SKILL.md"),
        "old",
      );
  ...
  describe("skill content validation", () => {
    ...
    it("should include proper instructions in skill files", async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, ".claude", "skills");
      await fs.mkdir(path.join(skillsDir, "phspec-apply-change"), {
        recursive: true,
      });
      await fs.writeFile(
...
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("Removed PhSpec markers from CLAUDE.md"),
      );
...
`;
  ...
  describe("legacy tool upgrade", () => {
    it("should upgrade legacy tools to new skills with --force", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("/phsx:new"),
      );
    ...
    it("should create commands when upgrading legacy tools", async () => {
      ...
      // New phsx commands should be created
      const commandsDir = path.join(testDir, ".claude", "commands", "phsx");
      const exploreCmd = path.join(commandsDir, "explore.md");
      const exists = await FileSystemUtils.fileExists(exploreCmd);
      expect(exists).toBe(true);
    });
