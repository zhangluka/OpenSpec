...
describe("instruction-loader", () => {
  describe("loadTemplate", () => {
    ...
    it("should include template path in error", () => {
      try {
        ...
        expect((err as TemplateLoadError).templatePath).toContain(
          "nonexistent.md",
        );
      }
    });
  ...
  describe("loadChangeContext", () => {
    ...
    it("should detect completed artifacts", () => {
      ...
      const changeDir = path.join(tempDir, "phspec", "changes", "my-change");
    ...
    it("should auto-detect schema from .phspec.yaml metadata", () => {
      // Create change directory with metadata file
      const changeDir = path.join(tempDir, "phspec", "changes", "my-change");
      fs.mkdirSync(changeDir, { recursive: true });
      fs.writeFileSync(
        path.join(changeDir, ".phspec.yaml"),
        'schema: spec-driven\ncreated: "2025-01-05"\n',
      );
  ...
  describe("generateInstructions", () => {
    ...
    describe("project config integration", () => {
      it("should return context as separate field for all artifacts", () => {
        ...
        const configDir = path.join(tempDir, "phspec");
...
`,
        ...
        const proposalInstructions = generateInstructions(
          context,
          "proposal",
          tempDir,
        );
        expect(proposalInstructions.rules).toEqual([
          "Include rollback plan",
          "Identify affected teams",
        ]);
        ...
        const specsInstructions = generateInstructions(
          context,
          "specs",
          tempDir,
        );
...
`,
        ...
        const designInstructions = generateInstructions(
          context,
          "design",
          tempDir,
        );
...
`,
      ...
      it("should deduplicate validation warnings within session", () => {
        ...
        const freshTempDir = fs.mkdtempSync(
          path.join(os.tmpdir(), "openspec-test-"),
        );

        try {
          // Create project config with a uniquely named invalid artifact ID
          const configDir = path.join(freshTempDir, "phspec");
          fs.mkdirSync(configDir, { recursive: true });
          fs.writeFileSync(
            path.join(configDir, "config.yaml"),
            `schema: spec-driven
