...
describe("telemetry/config", () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `phspec-telemetry-test-${Date.now()}`);
  ...
  describe("getConfigPath", () => {
    it("should return path to config.json in .config/phspec", () => {
      const result = getConfigPath();
      expect(result).toBe(
        path.join(tempDir, ".config", "phspec", "config.json"),
      );
    });
  });
  ...
  describe("writeConfig", () => {
    ...
    it("should write config to file", async () => {
      const configPath = path.join(tempDir, ".config", "phspec", "config.json");
