...
/**
 * Generates Zsh completion scripts for the PhSpec CLI.
 * Follows Zsh completion system conventions using the _phspec function.
 */
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      commandCaseLines.push(
        `          _phspec_${this.sanitizeFunctionName(cmd.name)}`,
      );
      commandCaseLines.push("          ;;");
    }
    ...
    return `#compdef phspec

# Zsh completion script for PhSpec CLI
...
${commandList}
  ...
  case $state in
    command)
      _describe "phspec command" commands
      ;;
    args)
      case $words[1] in
${commandCases}
...
`;
  ...
  private generateCommandFunction(cmd: CommandDefinition): string[] {
    const funcName = `_phspec_${this.sanitizeFunctionName(cmd.name)}`;
    const lines: string[] = [];
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        ...
        lines.push(
          `          _phspec_${this.sanitizeFunctionName(cmd.name)}_${this.sanitizeFunctionName(subcmd.name)}`,
        );
        lines.push("          ;;");
      }
  ...
  private generateSubcommandFunction(
    ...
    const funcName = `_phspec_${this.sanitizeFunctionName(parentName)}_${this.sanitizeFunctionName(subcmd.name)}`;
    const lines: string[] = [];
  ...
  private generatePositionalSpec(positionalType?: string): string {
    switch (positionalType) {
      case "change-id":
        return "'*: :_phspec_complete_changes'";
      case "spec-id":
        return "'*: :_phspec_complete_specs'";
      case "change-or-spec-id":
        return "'*: :_phspec_complete_items'";
      case "path":
        return "'*:path:_files'";
      case "shell":
