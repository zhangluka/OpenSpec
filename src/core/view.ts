...
export class ViewCommand {
  async execute(targetPath: string = "."): Promise<void> {
    ...
    if (specsData.length > 0) {
      ...
      specsData.forEach((spec) => {
        const reqLabel = spec.requirementCount === 1 ? "条需求" : "条需求";
        console.log(
  ...
  private displaySummary(
    ...
    const totalChanges =
      changesData.draft.length +
      changesData.active.length +
      changesData.completed.length;
    ...
    console.log(
      `  ${chalk.cyan("●")} 规范：${chalk.bold(totalSpecs)} 条，${chalk.bold(totalRequirements)} 条需求`,
    );
    if (changesData.draft.length > 0) {
      console.log(
        `  ${chalk.gray("●")} 草稿变更：${chalk.bold(changesData.draft.length)}`,
      );
    }
    console.log(
      `  ${chalk.yellow("●")} 进行中的变更：${chalk.bold(changesData.active.length)} 项`,
    );
    console.log(
      `  ${chalk.green("●")} 已完成的变更：${chalk.bold(changesData.completed.length)}`,
    );

    if (totalTasks > 0) {
      const overallProgress = Math.round((completedTasks / totalTasks) * 100);
      console.log(
        `  ${chalk.magenta("●")} 任务进度：${chalk.bold(`${completedTasks}/${totalTasks}`)}（${overallProgress}% 已完成）`,
      );
    }
  }
