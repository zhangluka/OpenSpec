...
echo "Test 3: PHSPEC_NO_COMPLETIONS=1 (should skip silently)"
echo "--------------------------------------"
unset CI
export PHSPEC_NO_COMPLETIONS=1
node scripts/postinstall.js
...
if [ -n "$ORIGINAL_PHSPEC_NO_COMPLETIONS" ]; then
  export PHSPEC_NO_COMPLETIONS="$ORIGINAL_PHSPEC_NO_COMPLETIONS"
else
  unset PHSPEC_NO_COMPLETIONS
fi
